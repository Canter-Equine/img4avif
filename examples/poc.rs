...
//! RUST_LOG=img4avif=debug cargo run --example poc --features dev-logging
//! ```
...
///
/// Returns `Ok(ConvResult)` on success or the `img4avif::Error` on failure.
fn do_convert(
    data: &[u8],
    stem: &str,
    suffix: &str,
...
fn main() {
    println!("img4avif proof-of-concept");
    println!("=========================");
    println!();
