...
impl Converter {
    ...
    pub fn convert_multi(&self, input: &[u8]) -> Result<Vec<ConversionOutput>, Error> {
        ...
        let (last_res, rest_res) = resolutions.split_last().expect(
            "resolutions list is guaranteed non-empty by earlier normalisation to [Original]",
        );
        ...
        let data = match self.encode_raw(&resized) {
            Ok(d) => d,
            Err(e) => {
                img_error!("convert_multi: encode for {:?} failed — {}", last_res, e);
                return Err(e);
            }
        };
